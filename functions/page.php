<?php
     ...
     */
     ...
     function get_contents_related_category($id, $type = 'category-content'){
        $categoryHtml = '';
        $term_list = get_the_terms($id, $type);
        if(!empty($term_list)){
            $categoryHtml .='<dd>';
            foreach($term_list as $item){
                $categoryHtml .= sprintf(
                    '<span class="m-categoryListSecondary__txt">%s</span>',
                    $item->name
                );
            }
            $categoryHtml .='</dd>';
     ...
     */
     ...
     function get_news_related_category($id, $type = 'category-content'){
        $categoryHtml = '';
        $term_list = get_the_terms($id, $type);
        if(!empty($term_list)){
            $categoryHtml .='<div class="m-newsList__item--category">';
     ...
     */
     ...
     function get_detail_contents_category($id){
        ...
        if(!empty($term_list)){
            ...
            foreach($term_list as $item){
                $categoryHtml .= sprintf(
                    '<a href="%s" class="m-categoryListPrimary__txt">%s</a>',
     ...
     function get_detail_contents_category_name($id){
        ...
        if(!empty($term_list)){
            ...
            foreach($term_list as $key => $item){
                $categoryHtml .= sprintf(
                    '<span class="m-categoryListPrimary__txt">%s</span>',
                    $item->name
                );
            }
            $categoryHtml .= '</dd>';
     ...
     */
     ...
     function get_contents_related_from_category($post_id, $post_type, $taxonomy_name){
        ...
        $txt_related = ($post_type == 'post')?'related_post':'related_content';
        if (class_exists('Smart_Custom_Fields') && ($related_content = SCF::get($txt_related, $post_id)) && !empty($related_content)) {
                ...
                while ( $dataContent->have_posts() ) : $dataContent->the_post(); ?>
                    ...
                    <a class="m-contentsRelatedList__item--block" href="<?php the_permalink() ?>">
                        ...
                        <div class="m-contentsRelatedList__item--wrap">
                            <dl class="m-categoryListSecondary">
                                <dt class="m-categoryListSecondary__txt"><?php echo get_the_date( 'Y.m.d'); ?></dt>
                                <?php echo get_contents_related_category(get_the_ID(), $taxonomy_name); ?>
                            </dl>
                        </div>
                        </div>
                    </a>
     ...
     */
     ...
     function get_contents_related_same_taxonomy($post_id, $post_type, $taxonomy_name){
        ...
        $txt_related = ($post_type == 'post')?'related_post':'related_content';
        if (class_exists('Smart_Custom_Fields') && ($related_posts = SCF::get($txt_related)) && !empty($related_posts)) {
                ...
                <li class="m-newsList__item">
                    <a class="m-newsList__item--block" href="<?php the_permalink() ?>">
                        ...
                        <div class="m-newsList__item--wrap">
                            ...
                            <?php echo get_news_related_category(get_the_ID(), $taxonomy_name); ?>
                        </div>
                        </div>
                    </a>
                    </li>
