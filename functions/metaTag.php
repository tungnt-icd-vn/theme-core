...
function get_seo_post_image() {
  ...
  return !empty($meta_image) ? wp_get_attachment_image_url($meta_image, 'full') : null;
}
...
function get_meta_title() {
  $bloginfo = '株式会社カインズ 学生インターンシップ 2023';
  ...
  if (is_404()) {
    $title_seo = 'ページが見つかりません'.implode(' | ', $title_default);
  }
...
add_action('wp_head', 'gen_meta_image', 5);
...
add_action('wp_head', 'gen_meta_description', 4);
...
add_action('wp_head', 'gen_meta_keywords', 6);